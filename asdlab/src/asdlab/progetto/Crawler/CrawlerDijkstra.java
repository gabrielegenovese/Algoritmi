...
import asdlab.libreria.CamminiMinimi.*;

/* ============================================================================
 *  $RCSfile: CrawlerDijkstra.java,v $
 * ============================================================================
...
/**
 * La classe <code>CrawlerDijkstra</code> implementa un crawler basato sull'algoritmo
 * per i cammini minimi di Dijkstra. In particolare, ad ogni iterazione, l'attivit&agrave;
 * di crawling opera scegliendo come prossima pagina da acquisire quella 
 * per cui esiste il link di costo minimo tra quelli che collegano 
 * pagine gi&agrave; acquisite con pagine da acquisire. Il costo di un arco
 * &egrave; determinato dalla funzione {@link CostoLink#costo(Object, Object)} 
 * dell'interfaccia {@link CostoLink}, di cui deve essere indicata una implementazione
 * all'atto dell'allocazione della classe <code>CrawlerDijkstra</code>.
 *  Le pagine esaminate durante la visita vengono acquisite e memorizzate
 * su disco per poi essere successivamente indicizzate. La classe  <code>CrawlerDijkstra</code>
 * &egrave; realizzata derivando la classe {@link Dijkstra}
 * e ridefinendo il metodo {@link VisitaBFS#visita(Nodo u)}.
 ...
 * e viene rivelata durante la visita stessa. Per questo motivo, la classe <code>CrawlerBFS</code>
 * specializza il metodo {@link Dijkstra#visita(Nodo u)} di {@link Dijkstra},
...
public class CrawlerDijkstra extends Dijkstra implements Crawler {
	...
	/**
	 * Riferimento alla implementazione dell'interfaccia <code>CostoLink</code> utilizzata per associare 
	 * un costo a ciascun arco
	 */
	private CostoLink cl;
	
	/**
	 * Costruttore per l'allocazione di una nuova istanza di <code>CrawlerDijkstra</code>.
	 * 
	 * @param cl il riferimento alla implementazione dell'interfaccia <code>CostoLink</code> da utilizzare per associare 
	 * un costo a ciascun arco
	 */
	public CrawlerDijkstra(CostoLink cl) { this.cl = cl; }
	...
	public Grafo crawl(String urlIniz, String dirArchivio, 
				int maxDocs, boolean compresso){
	...
	/**
	 * Visita il nodo di input <code>u</code> secondo la logica dell'algoritmo
	 * di Djikstra ed aggiorna la struttura del grafo di conseguenza. La
	 * visita avviene innanzitutto richiamando il metodo {@link Dijkstra#visita(Nodo u)}
	 * di {@link Dijkstra} per visitare il nodo <code>u</code> secondo
	 * la logica dell'algoritmo di Djikstra. Successivamente, si acquisisce tramite l'oggetto
	 ...
	 * al grafo sotto forma di archi. Il costo di ciascun arco
	 * &egrave; attribuito mediante il metodo {@link CostoLink#costo(Object, Object)} 
	 * di <code>cl</code>. I nodi raggiunti per la prima volta
	...
	protected void visita(Nodo u) { 
		...
		while (i.hasNext() && numDocs < maxDocs){
			...
			double costo = cl.costo(urlU, urlV);
			if (costo == -1) continue;
